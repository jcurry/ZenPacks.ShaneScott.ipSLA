# Two changes made by Jane Curry to make this work - search for JC comments
...
class SLAPerformanceConfig(CollectorConfigService):
    ...
    def _transform_oid(self, oid, comp):
        ...
        # JC - debugging
        # SLAindex_dct is always None
        #self.log.info(' in _transform_oid oid is %s and comp is %s and SLAindex_dct is %s \n' % (oid, comp, SLAindex_dct))
        ...
        if SLAindex_dct is not None:
            for prefix, index_ in SLAindex_dct.iteritems():
                if oid.startswith(prefix):
                    ...
                    break
        if index is None:
            # JC change - ipSLAs components have their index in the instance attribute, not in snmpindex
            #index = getattr(comp, "ifindex", comp.snmpindex)
            index = getattr(comp, "ifindex", comp.instance)
            #self.log.info(' in _transform_oid index is %s \n' % (index))
    ...
    def _createDeviceProxies(self, device):
        ...
        # Next line changed by JC
        #components = device.os.getMonitoredComponents(collector="zensla")
        components = device.os.getMonitoredComponents(type="SLAs")
